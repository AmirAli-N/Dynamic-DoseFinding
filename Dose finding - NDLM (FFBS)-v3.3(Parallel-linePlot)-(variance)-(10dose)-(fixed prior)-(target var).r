...
true_theta=c(10, 10.5, 11.5, 14, 18, 24, 28, 30, 31, 31.5, 32)
true_sigma=1
...
FFBS_sampler <- function(y, iter, thin){
	...
	return (list("params"=Params, "var"=C[[11]][1,1])) #for curve1: 6 for curve2: 11, curve3: 9
}
...
dose_allocation <- function(y){
	...
	return (list("variance"=as.vector(unlist(lapply(var_j,mean))), "theta"=theta_estimate, "target_var"=res$var))
}
start_time=Sys.time()
for (k in 1:patient){
	if (k==1){
	...
	#other patients
		...
		var_target=c(var_target, res$target_var)
	}
}
end_time=Sys.time()
start_time-end_time
write(var_target, file="C:/Users/snasrol/Google Drive/Research-Dose finding/Codes/Results-10dose/08.21.2017/NDLM-curve2target_var.txt", append=FALSE, sep="\n")
write(dose, file="C:/Users/snasrol/Google Drive/Research-Dose finding/Codes/Results-10dose/08.21.2017/NDLM-curve2doses.txt", append=FALSE, sep="\n")
write.table(var_estimate, file="C:/Users/snasrol/Google Drive/Research-Dose finding/Codes/Results-10dose/08.21.2017/NDLM-curve2var.txt", sep="\t", eol="\n", row.names=FALSE, col.names=FALSE)
write.table(theta_estimate, file="C:/Users/snasrol/Google Drive/Research-Dose finding/Codes/Results-10dose/08.21.2017/NDLM-curve2thetas.txt", sep="\t", eol="\n", row.names=FALSE, col.names=FALSE)
plot(dose[1:patient])
